 ...
 */
export class ID3Container {
  private onDoneCallback: (error?: Error) => void;
  private readStream: fs.ReadStream;
  private isHeaderRead = false;
  private readonly framesById: Map<string, ID3Frame> = new Map();
  private currentFrame: ID3Frame|null = null;
  private isDone = false;
  private error: Error|undefined = undefined;
  private version = 3;
  private revision = 0;
  private flags = 0;
  private originalTagSize = 0;
  private paddingSize = 0;
  private tagBytesRemaining = 0;
  ...
  constructor(container: string|fs.ReadStream) {
    ...
    if (containerType == 'string') {
      ...
      this.readStream = container as fs.ReadStream;
    }
   ...
   */
  onDone(callback: (error?: Error) => void) {
   ...
   * be found.
   */
  getFrame(id: ID3FrameId, createIfMissing: boolean = false): ID3Frame|null {
    if (createIfMissing && !this.framesById.has(id)) {
      this.framesById.set(id, new ID3Frame(id));
    }
   ...
   */
  write(callback: (error?: Error) => void, container?: string|fs.WriteStream) {
    const containerType = typeof container;
    ...
    readStream.on('error', (err) => {
      console.error(
          'Error reading file on ID3 write: ' + container +':' + err);
      callback(err);
      if (readStream) {
        readStream.close();
        readStream = null;
    ...
    writeStream.on('error', (err) => {
      console.error(
          'Error writing file on ID3 write: ' + container +':' + err);
      callback(err);
      if (readStream) {
        readStream.close();
        readStream = null;
  ...
  private onReadable() {
    ...
    try {
      ...
      if (this.paddingSize) {
        ...
        // well.
        const remainingBytes: Buffer = this.readStream.read(this.tagBytesRemaining);
        if (remainingBytes) {
   ...
   */
  private writeHeader(writeStream: Writable, totalSize: number): void {
  ...
  private parseFrame(): boolean {
    if (!this.currentFrame) {
      ...
      this.currentFrame = new ID3Frame(frameHeader, this.version);
    }
